...
public class ReplayController {
    ...
    public Page<ReplayJobSummaryDTO> listReplays(
            @PathVariable UUID eventId,
            @AuthenticationPrincipal String hookSecret,
            Pageable pageable
    ) {
    ...
    @GetMapping("/replays/{replayJobId}/attempts")
    public Page<DeliveryAttemptDTO> listDeliveryAttempts(
            @PathVariable UUID replayJobId,
            @AuthenticationPrincipal String secret,
            Pageable pageable
    ) {
        return replayJobService.listAttempts(replayJobId, secret, pageable);
    }
}
